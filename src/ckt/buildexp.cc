...
//#endif
	...
	print_mesg("Construct fcoeff ddd from ddd ...");
    if(!matrix->index_mat)
	{
		theCkt->build_mna();
	}
    ...
    if(!theFLabMap)
	{
        fcoeff_build_expressions_from_ddd(decancel);
	}
//#ifdef _DEBUG
	print_mesg(" frequence sweep ...");
	double time2;
    double c_time;
    time1 = 1.0 * clock() / CLOCKS_PER_SEC;
    fcoeff_freq_sweep(FALSE);
    time2 = 1.0 * clock() / CLOCKS_PER_SEC;
    c_time = time2 - time1;
    cout <<"Evaluation time time: " << c_time <<endl;
    //graph_display();
	print_mesg(" end of frequence sweep");
//#endif
...
)
{
    ...
    print_mesg("Construct fcoeff ddd from simplified ddd ...");
    if(!matrix->index_mat)
	{
        theCkt->build_mna();
	}
    ...
    if(!theFLabMap)
	{
        fcoeff_build_expressions_from_sddd(decancel);
	}
...
)
{
    ...
    //build the de-cancellation map if decancellation is
	// required.
	if(decancel)
	{
		theCkt->fcoeff_new_decancel();
	}
...
-----------------------------------------------------*/
DDDnode *
ACAnalysisManager::build_cofactor_ddd
( 
	int row, 
	int col 
)
